 ...
 * CLAUDE.md).
 *
 * Entre lo público hay un constructor de extractos sintéticos por formato
 * (`norma43Bytes`, `revolutCsvBytes`), para las capas de encima: el borde HTTP
 * y `pnpm seed`. Salen por aquí en vez de importarse de `adapters/` a pelo
 * porque la frontera del módulo vale también para los tests y para la semilla.
 * Que un test de la ruta necesite un cuaderno 43 de verdad no es motivo para
 * saltársela, es motivo para que el módulo ofrezca uno.
 */
...
export {
  ...
  type SyntheticStatement as SyntheticNorma43Statement,
...
export {
  revolutCsvBytes,
  type SyntheticMovement as SyntheticRevolutMovement,
  type SyntheticStatement as SyntheticRevolutStatement,
} from './adapters/revolut-csv/testing'
