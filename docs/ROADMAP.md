...
- [x] Puesta en marcha con datos reales: dejar de desarrollar contra la semilla. Tres huecos que solo se ven al intentarlo de verdad, ninguno en el dominio. **Uno**: el fichero que exporta Unicaja se llama `.csb` —por el antiguo Consejo Superior Bancario— y el `accept` del selector solo ofrecía `.n43`, así que en el móvil el extracto salía agrisado y no se podía ni elegir. El parser no necesitaba nada: es el cuaderno 43 de siempre y `verifyIntegrity()` cuadra al céntimo contra un fichero real. El `accept` pasa a derivarse del formato ya elegido, como `Record` completo igual que `SOURCE_LABELS`, así que un formato nuevo no compila hasta que se decide qué extensiones ofrece. **Dos**: `pnpm seed --reset` recreaba la base pero volvía a sembrarla entera, y no había forma de quedarse con una base limpia; `--empty` siembra solo el vocabulario —categorías y reglas— y nada de contenido. Implica el borrado en vez de exigir `--reset` al lado, porque vaciar una base ya poblada no se consigue sembrando menos y pedir dos banderas solo abre la puerta a ejecutar media operación; la guarda de `.dev/` sigue intacta. **Tres**: el alta de cuenta no preguntaba el saldo inicial, que es la base del invariante 6, así que todos los saldos salían desplazados. `parseMoneyCents` se escribe en la web y no se importa de `packages/core` porque la regla `web-only-shared` lo prohíbe (ADR-009), y hace la conversión con manipulación de cadenas y no multiplicando por cien, que es la operación que mete el error de coma flotante; un test de ida y vuelta con `formatMoney` es lo que detectaría que los dos se separen. El texto de ayuda del campo es parte del entregable: si se escribe el saldo de hoy en vez del anterior al primer movimiento, la app queda mal y **ningún test lo detecta**. De propina, `.gitignore` aprende las extensiones de extracto: el hook pre-commit busca IBANes (`ES` + 22 dígitos) y un cuaderno 43 trae entidad, oficina y cuenta sueltas, sin prefijo de país, así que uno caído fuera de `data/` se colaba por las dos defensas a la vez
