...
- `data/`, `*.db` (y sus `-wal`/`-shm`) están en `.gitignore`, y también las extensiones de extracto (`.csb`, `.n43`, `.q43`, `.c43`, `.aeb`) para cazar el que se descarga y se arrastra a la carpeta equivocada: el hook busca IBANes (`ES` + 22 dígitos) y un cuaderno 43 trae entidad, oficina y cuenta sueltas, sin prefijo de país, así que uno caído fuera de `data/` se colaría por las dos defensas a la vez. Los extractos reales van siempre a `data/`, nunca a otra carpeta.
- **Este repositorio es público** (`github.com/AlexFandila/ecodata`), así que «cero datos reales en git» no es una precaución por si acaso: es el requisito del que dependen todos los demás. Y no basta con los ficheros: una auditoría encontró fixtures de tests que reproducían importes de un extracto real —la nómina emparejada con el concepto literal del banco es una nómina reconstruida—, y hubo que reescribirlos. Al escribir tests, inventa las cifras aunque tengas delante el fichero de verdad.
